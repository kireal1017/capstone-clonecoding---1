# OMC 단계별 실행 명령어 모음 - 스마트 점검 리포트

> 이 문서는 OMC 또는 Claude Code 세션에서 단계별로 붙여넣어 사용할 작업 지시문 모음입니다.  
> 실제 OMC의 명령어 형식은 설치/설정 방식에 따라 다를 수 있으므로, 여기서는 **각 세션에 입력할 프롬프트형 명령어**를 기준으로 정리합니다.

---

## 0. 문서 배치 기준

OMC 세션을 시작하기 전에 프로젝트의 `docs/` 디렉토리는 아래 구조로 정리되어 있다고 가정합니다.
...
각 OMC/Claude Code 세션을 시작할 때 아래 공통 문장을 먼저 포함합니다.
...
- 완료된 세션, 보류된 이슈, 다음 세션 인수인계를 확인한다.
- 이전 세션에서 이미 완료한 작업을 중복 구현하지 않는다.

작업 종료 후:
- 반드시 `docs/progress.md`를 갱신한다.
- 전체 진행 현황 표의 이번 세션 상태를 갱신한다.
- 세션 기록 요약과 상세 세션 기록에 이번 작업 내용을 추가한다.
- 수정 파일, 실행 명령, 검증 결과, 실패/보류/TODO, 다음 세션 인수인계를 기록한다.

작업 완료 후 반드시 다음을 보고하라.
1. 구현한 내용
2. 수정한 파일
3. 실행 방법
4. 검증 결과
5. 다음 세션에서 이어받아야 할 내용
6. 실패/보류/임시 처리한 내용
```
...
반드시 참고할 문서:
1. docs/prd/PRD_v1.0_스마트점검리포트.md
2. docs/prd/프론트엔드_PRD_세션별_v2_와이어프레임반영.md
3. docs/prd/백엔드_PRD_세션별.md
4. docs/harness/개발_하네스_OMC_ClaudeCode_v2_DESIGN반영.md
...
1. docs/prd/PRD_v1.0_스마트점검리포트.md
2. docs/prd/백엔드_PRD_세션별.md
3. docs/specs/기능명세서_v3.0_스마트점검리포트.md
4. docs/harness/개발_하네스_OMC_ClaudeCode_v2_DESIGN반영.md
...
5. docs/harness/개발_하네스_OMC_ClaudeCode_v2_DESIGN반영.md
...
반드시 참고할 문서:
1. docs/prd/프론트엔드_PRD_세션별_v2_와이어프레임반영.md
...
## 18-1. F-VERIFY - Playwright MCP 프론트엔드 전체 검증
...
각 세션이 끝나면 아래 형식으로 반드시 보고하도록 지시한다.
